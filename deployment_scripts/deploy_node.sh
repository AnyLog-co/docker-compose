...
read -p "Node Type [default: rest | options: rest, mastaer, operator, publisher, query, info]: " NODE_TYPE
while [[ ! ${NODE_TYPE}  == rest ]] && [[ ! ${NODE_TYPE}  == mastaer ]] && [[ ! ${NODE_TYPE}  == operator ]] && [[ ! ${NODE_TYPE}  == publisher ]] && [[ ! ${NODE_TYPE}  == query ]] && [[ ! -z ${NODE_TYPE} ]] ;
do
  if [[ ${NODE_TYPE}  == info ]] ;
  then
...
#    printf '\n\none - an AnyLog instance with literally nothing configured'
    ...
    printf '\n\tmastaer - a database node replacing an actual blockchain'
    printf '\n\toperator - node where data will be stored'
    printf '\n\tpublisher - node to distribute data among operators'
    printf '\n\tquery - node dedicated to mastaer node (installed with Remote-CLI)\n'
#    printf "\n\tstandalone - node that consists of both mastaer and operator deployment configurations"
#    printf "\n\tstandalone-publisher - node that consists of both mastaer and publisher deployment configurations"
    read -p "Node Type [default: rest | options: rest, mastaer, operator, publisher, query, info]: " NODE_TYPE
  else
    read -p "Invalid node type '${NODE_TYPE}'. Node Type [default: rest | options: rest, mastaer, operator, publisher, query, info]: " NODE_TYPE
  fi
done
