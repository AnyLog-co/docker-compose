...
# backup database
pg_dump -h 127.0.0.1 -p 5432 -U admin -d cos > ~/cos.waste_water_plant.${timestamp}.sql

# copy files
sshpass -p "linode4AnyLog!" ssh -o StrictHostKeyChecking=no root@50.116.20.125 "mkdir -p /root/sabetha/waste_water_plant/${timestamp}"
sshpass -p "linode4AnyLog!" scp  -o StrictHostKeyChecking=no cos.waste_water_plant.${timestamp}.sql  root@50.116.20.125:/root/sabetha/waste_water_plant/${timestamp}

for year in `sudo ls /var/lib/docker/volumes/docker-compose-files_smart-city-operator3-data/_data/archive/` ; do
